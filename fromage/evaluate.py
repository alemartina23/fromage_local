...
def validate(val_loader, model, tokenizer, criterion, epoch, model_modes, args):
    ...
    def run_validate(loader, base_progress=0):
        with torch.no_grad():
            ...
            for i, (
                ...
                tgt_tokens,
                token_len,
            ) in tqdm.tqdm(enumerate(loader), position=0, total=len(loader)):
                i = base_progress + i
                ...
                for model_mode in model_modes:
                    (model_output, full_labels, last_embedding, _, visual_embs) = model(
                        ...
                        token_len,
                        mode=model_mode,
                        input_prefix=args.input_prompt,
                        inference=True,
                    )  # (N, T, C)
                    ...
                    if model_mode == "captioning":
                        acc1, acc5 = utils.accuracy(
                            output[:, :-1, :], full_labels[:, 1:], -100, topk=(1, 5)
                        )
                    ...
                    if model_mode == "captioning":
                        ...
                        if args.distributed:
                            ...
                            all_visual_embs = [
                                torch.zeros_like(visual_embs)
                                for _ in range(dist.get_world_size())
                            ]
                            all_last_embedding = [
                                torch.zeros_like(last_embedding)
                                for _ in range(dist.get_world_size())
                            ]
                            ...
                            assert torch.all(
                                last_embedding[start_idx:end_idx]
                                == original_last_embedding
                            ), args.rank
                    ...
                    if model_mode == "captioning":
                        input_embs = model.module.model.get_visual_embs(
                            images, mode="captioning"
                        )  # (2, n_visual_tokens, D)
                        if args.input_prompt is not None:
                            print(
                                f'Adding prefix "{args.input_prompt}" to captioning generate=True.'
                            )
                            ...
                            prompt_embs = model.module.model.input_embeddings(
                                prompt_ids
                            )
                        ...
                        gt_captions = tokenizer.batch_decode(
                            all_tgt_tokens, skip_special_tokens=True
                        )
                        ...
                        for cap_i in range(len(generated_captions)):
                            ...
                            if stop_idx > 5:
                                all_generated_captions.append(
                                    generated_captions[cap_i][:stop_idx]
                                )
                            else:
                        ...
                        if i == 0:
                            ...
                            input_ids = tgt_tokens[
                                :, :3
                            ]  # Use first 3 tokens as initial prompt for generation.
                            input_embs = model.module.model.input_embeddings(
                                input_ids
                            )  # (N, T, D)
                            generated_ids, _, _ = model(
                                input_embs,
                            ...
                            gt_captions = tokenizer.batch_decode(
                                tgt_tokens, skip_special_tokens=False
                            )
                    else:
                        raise NotImplementedError
                    ...
                    if i == 0:
                        ...
                        for cap_i, cap in enumerate(
                            generated_captions[:max_to_display]
                        ):
                            print(f"{cap_i}) {cap}")
                        print("=" * 30)
                        print("Real samples:")
                        ...
                        if not args.distributed or (args.rank % ngpus_per_node == 0):
                            ...
                            generated_cap_images = torch.stack(
                                [
                                    ...
                                    for j in range(normalized_images.shape[0])
                                ],
                                axis=0,
                            )
                            ...
                            display_images = torch.cat(
                                [
                                    normalized_images.float().cpu(),
                                    caption_images,
                                    generated_cap_images,
                                ],
                                axis=2,
                            )[:max_images_to_show]
                            grid = torchvision.utils.make_grid(
                                display_images,
                                nrow=int(max_images_to_show**0.5),
                                padding=4,
                            )
                            writer.add_image(
                                f"val/images_{model_mode}", grid, actual_step
                            )
            ...
            full_gt_captions = [
                path2captions[path] for path in all_generated_image_paths
            ]

            print(
                f"Computing BLEU with {len(all_generated_captions)} generated captions:"
            ...
            all_image_features = torch.cat(
                all_image_features, axis=0
            )  # (coco_val_len, 2048)
            all_text_features = torch.cat(
                all_text_features, axis=0
            )  # (coco_val_len, 2048)

            print(
                f"Computing similarity between {all_image_features.shape} and {all_text_features.shape}."
            )
            logits_per_image = all_image_features @ all_text_features.t()
            logits_per_text = logits_per_image.t()
            all_image_acc1, all_image_acc5 = losses_utils.contrastive_acc(
                logits_per_image, topk=(1, 5)
            )
            all_caption_acc1, all_caption_acc5 = losses_utils.contrastive_acc(
                logits_per_text, topk=(1, 5)
            )
    ...
    writer.add_scalar(
        "val/top1_acc", (top1_caption.avg + top1_image.avg) / 2.0, actual_step
    )
    writer.add_scalar(
        "val/top5_acc", (top5_caption.avg + top5_image.avg) / 2.0, actual_step
    )
