...
def create_image_of_text(
    text: str, width: int = 224, nrows: int = 2, color=(255, 255, 255), font=None
) -> torch.Tensor:
    ...
    cap_img = F.convert_image_dtype(
        F.pil_to_tensor(cap_img), torch.float32
    )  # (3, height, W * nrows)
    cap_img = torch.split(
        cap_img, effective_width, dim=-1
    )  # List of nrow elements of shape (3, height, W)
...
def get_feature_extractor_for_model(
    model_name: str, image_size: int = 224, train: bool = True
):
...
def get_pixel_values_for_model(feature_extractor, img):
    pixel_values = feature_extractor(
        img.convert("RGB"), return_tensors="pt"
    ).pixel_values[
        0, ...
    ]  # (3, H, W)
    return pixel_values
...
def accuracy(output, target, padding, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        if output.shape[-1] < maxk:
            print(
                f"[WARNING] Less than {maxk} predictions available. Using {output.shape[-1]} for topk."
            )
