...
def get_dataset(args, split: str, tokenizer, precision: str = "fp32") -> Dataset:
    ...
    if split == "train":
        ...
        assert len(dataset_paths) == len(image_data_dirs) == 1, (
            dataset_paths,
            image_data_dirs,
        )
    else:
        ...
        dataset = torch.utils.data.ConcatDataset(
            [
                CsvDataset(
                    ...
                    retrieval_token_idx=args.retrieval_token_idx,
                )
    ...
    def __getitem__(self, idx):
        while True:
            ...
            try:
                ...
                decode_caption = self.tokenizer.decode(
                    tokens, skip_special_tokens=False
                )
                self.font = self.font or ImageFont.load_default()
                cap_img = utils.create_image_of_text(
                    decode_caption.encode("ascii", "ignore"),
