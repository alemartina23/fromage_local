...
from fromage.config import HF_CACHE_DIR
...
class FromageModel(nn.Module):
    def __init__(self, tokenizer, args: FrozenArgs = FrozenArgs()):
        ...
        assert args.text_emb_layers != set(
            args.text_emb_layers
        ), "text_emb_layers not unique"
        self.args = args
        ...
        print(
            f"Using {visual_encoder} for the visual model with {n_visual_tokens} visual tokens."
        )

        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))

        if "facebook/opt" in opt_version:
            self.lm = OPTForCausalLM.from_pretrained(
                opt_version, cache_dir=HF_CACHE_DIR
            )
        else:
        ...
        print(
            f"Initializing embedding for the retrieval token [RET] (id = {self.retrieval_token_idx})."
        )
        ...
        if "clip" in visual_encoder:
            self.visual_model = CLIPVisionModel.from_pretrained(
                visual_encoder, cache_dir=HF_CACHE_DIR
            )
        else:
            self.visual_model = AutoModel.from_pretrained(
                visual_encoder, cache_dir=HF_CACHE_DIR
            )
        ...
        if self.args.shared_emb_dim is None:
            if len(self.args.text_emb_layers) == 1:
                if (
                    self.args.text_emb_layers[0]
                    in [-1, self.lm.config.num_hidden_layers]
                ) and ("bert" not in opt_version):
                    out_dim = self.lm.config.word_embed_proj_dim
                else:
                    out_dim = self.lm.config.hidden_size
            else:
                if (
                    (-1 in self.args.text_emb_layers)
                    or (self.lm.config.num_hidden_layers in self.args.text_emb_layers)
                    and (
                        self.lm.config.word_embed_proj_dim != self.lm.config.hidden_size
                    )
                ):
                    raise ValueError(
            ...
            for layer_idx in self.args.text_emb_layers:
                if (
                    layer_idx == -1 or layer_idx == self.lm.config.num_hidden_layers
                ) and ("bert" not in opt_version):
                    ...
                    text_fc = [
                        nn.Linear(self.lm.config.hidden_size, out_dim),
                        nn.Dropout(self.args.text_embed_dropout_prob),
                    ]
                    self.text_hidden_fcs.append(nn.Sequential(*text_fc))
                else:
    ...
    def get_visual_embs(
        self, pixel_values: torch.FloatTensor, mode: str = "captioning"
    ):
        if mode not in ["captioning", "retrieval"]:
            raise ValueError(
                f'mode should be one of ["caption", "retrieval"], got {mode} instead.'
            )
        ...
        if mode == "captioning":
            visual_embs = self.visual_embeddings(
                encoder_outputs
            )  # (2, D * n_visual_tokens)
            visual_embs = torch.reshape(
                visual_embs, (visual_embs.shape[0], self.args.n_visual_tokens, -1)
            )
        elif mode == "retrieval":
    ...
    def forward(
        ...
        last_embedding_idx = (
            caption_len - 1
        )  # -1 to retrieve the token before the eos token

        if input_prefix is not None:
            prompt_ids = self.tokenizer(
                input_prefix, add_special_tokens=False, return_tensors="pt"
            ).input_ids
        ...
        if mode == "captioning":
            ...
            if input_prefix is None:
                ...
                full_labels = (
                    torch.zeros(visual_embs.shape[:2], dtype=torch.int64).to(
                        visual_embs.device
                    )
                    - 100
                )
                ...
                full_labels = (
                    torch.zeros(prefix_embs.shape[:2], dtype=torch.int64).to(
                        visual_embs.device
                    )
                    - 100
                )
            ...
            if concat_captions:
                ...
                for i in range(batch_size // 2):
                    ...
                    second_labels_padding = full_labels[
                        second_idx, pad_idx[second_idx] :
                    ]

                    assert torch.all(first_labels_padding == -100), first_labels_padding
                    assert torch.all(
                        second_labels_padding == -100
                    ), second_labels_padding
                    concat_input_embs = torch.cat(
                        [first_emb, second_emb, first_padding, second_padding], axis=0
                    )  # (T*2, 768)
                    concat_labels = torch.cat(
                        [
                            first_labels,
                            second_labels,
                            first_labels_padding,
                            second_labels_padding,
                        ],
                        axis=0,
                    )  # (T*2, 768)
            ...
            output = self.lm(
                inputs_embeds=input_embs, labels=full_labels, output_hidden_states=True
            )
        elif mode == "retrieval":
        ...
        if mode == "retrieval":
            if self.args.shared_emb_dim is not None:
                for idx, fc_layer in zip(
                    self.args.text_emb_layers, self.text_hidden_fcs
                ):
                    hidden_states.append(
                        fc_layer(output.hidden_states[idx])
                    )  # (N, seq_len, 2048)
            else:
            ...
            if not concat_captions:
                last_embedding = torch.stack(
                    [
                        last_hidden_state[i, last_embedding_idx[i], :]
                ...
                last_output_logit = torch.stack(
                    [
                        output.logits[i, last_embedding_idx[i] - 1, :]
                        for i in range(batch_size)
                    ],
                    axis=0,
                )  # (N, D)
            ...
            for i in range(max_len):
                ...
                if self.args.shared_emb_dim is not None:
                    ...
                    for idx in self.args.text_emb_layers:
                        hidden_states.append(output.hidden_states[idx])
                # Add hidden states together.
                last_hidden_state = torch.stack(hidden_states, dim=-1).sum(
                    dim=-1
                )  # (N, T, 256)
                last_embedding = last_hidden_state / last_hidden_state.norm(
                    dim=-1, keepdim=True
                )
                ...
                if (
                    self.retrieval_token_idx != -1
                    and self.retrieval_token_idx is not None
                ):
                    if i < min_word_tokens:
                        ...
                        logits[:, self.retrieval_token_idx] = (
                            logits[:, self.retrieval_token_idx] * ret_scale_factor
                        )
                ...
                if temperature == 0.0:
                    if top_p != 1.0:
                        raise ValueError(
                            "top_p cannot be set if temperature is 0 (greedy decoding)."
                        )
                    ...
                    if top_p < 1.0:
                        assert (
                            top_p > 0
                        ), f"top_p should be above 0, got {top_p} instead."
                ...
                if "opt" in self.opt_version:
                    ...
                    and (next_token == self.tokenizer.eos_token_id).all()
                ):
                    # End of generation.
                    break
...
class Fromage(nn.Module):
    ...
    def __call__(
        ...
        caption_len: Optional[Tensor] = None,
        generate: bool = False,
        num_words: int = 32,
    ...
    def generate_for_images_and_texts(
        ...
        for i, p in enumerate(prompts):
            if type(p) == Image.Image:
                # Encode as image.
                pixel_values = utils.get_pixel_values_for_model(
                    self.model.feature_extractor, p
                )
                pixel_values = pixel_values.to(
                    device=self.model.logit_scale.device,
                    dtype=self.model.logit_scale.dtype,
                )
                pixel_values = pixel_values[None, ...]

                visual_embs = self.model.get_visual_embs(
                    pixel_values, mode="captioning"
                )  # (1, n_visual_tokens, D)
                input_embs.append(visual_embs)
            elif type(p) == str:
                text_ids = self.model.tokenizer(
                    p, add_special_tokens=True, return_tensors="pt"
                ).input_ids.to(self.model.logit_scale.device)
                if not add_bos:
                    # Remove <bos> tag.
                    text_ids = text_ids[:, 1:]
                else:
        ...
        if num_words == 0:
            ...
            outputs = self.model.lm(
                inputs_embeds=input_embs, use_cache=False, output_hidden_states=True
            )
            # Map outputs to embeddings, so we can retrieve embeddings from the [RET] tokens.
            out = []
            for x, fc in zip(
                self.model.args.text_emb_layers, self.model.text_hidden_fcs
            ):
            ...
            generated_ids, generated_embeddings, _ = self.model.generate(
                input_embs,
                num_words,
                temperature=temperature,
                top_p=top_p,
        ...
        all_ret_idx = [
            i
            for i, x in enumerate(generated_ids[0, :] == self.model.retrieval_token_idx)
            if x
        ][:max_num_rets]
        ...
        if len(all_ret_idx) == 0:
            # No [RET] tokens.
            caption = self.model.tokenizer.batch_decode(
            ...
            for ret_idx in all_ret_idx:
                ...
                caption = self.model.tokenizer.batch_decode(
                    generated_ids[:, last_ret_idx:ret_idx], skip_special_tokens=True
                )[0]
    ...
    def generate_for_images_and_texts_offline(
        self,
        prompts: List,
        num_words: int = 0,
        ret_scale_factor: float = 1.0,
    ...
    def get_log_lik_scores(self, prompts: List):
        """
...
def load_fromage(model_dir: str) -> Fromage:
    ...
    assert len(path_array) == emb_matrix.shape[0], (
        len(path_array),
        emb_matrix.shape[0],
    )
    ...
    tokenizer = GPT2Tokenizer.from_pretrained(
        model_kwargs["opt_version"], cache_dir=HF_CACHE_DIR
    )
    ...
    with torch.no_grad():
        model.model.input_embeddings.weight[model.model.retrieval_token_idx, :].copy_(
            checkpoint["state_dict"]["ret_input_embeddings.weight"]
            .squeeze()
            .cpu()
            .detach()
        )
    ...
    emb_matrix = torch.tensor(emb_matrix, dtype=logit_scale.dtype).to(
        logit_scale.device
    )
